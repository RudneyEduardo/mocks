            ...
            $.ajax(setCallbackSettings(access_token, $("#fname").val())).done(function (response) {
                ...
                for (var i = 2; i < 6; i++) {
                    var radioBtn = $("<label> R$" + (ValorPedido + ((ValorPedido / 100 * response.tax) * i)) +
                        " em " + i + "X com taxa de juros de " + response.tax + "% </label><input type='radio' value='"
                        + (ValorPedido + ((ValorPedido / 100 * response.tax) * i)) +
                        " em " + i + "X com taxa de juros de " + response.tax +
                        "' name='rbtnCount' /><br>");
...
$("#btnConfirmCredit").on("click", function () {
    ...
    $("#screen_credit_confirm p").text("A Teros Retail. está solicitando a" +
        "confirmação da compra de crédito no valor de:" + $('input[name=rbtnCount]:checked').val() + "%")
